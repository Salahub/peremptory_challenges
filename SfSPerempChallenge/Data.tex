...
Canadian data sets which examined jury selection in Canada could be found. The increased prominence of the jury selection process
...
\begin{quote}
   ...
   access to records is the single largest reason why jury selection cannot break out of the litigator's framework to become a
...
which served as the authoritative reference for the admissible factor levels of all variables. Solving this problem was as simple
...
minimize assumptions during data cleaning the occurrence of this outcome was replaced with ``U'' instead. Additionally, the
...
\begin{description}
  ...
  \item[Simplified Disposition] This variable combined the categories ``Foreman'' and ``Kept'' in the original disposition
    variable into a variable ``Kept''.
\end{description}
...
racial data for the two is recorded in a very similar way, so this effect can, at least, be compared.
...
performed on the data, rather variables were used and combined to generate analogous measures to those recorded by the Sunshine
...
the Sunshine data, and combining certain racial indicators into the more universal white/black/other coding.
...
jurors kept or peremptorily struck, venire members struck for cause were not included in the data. The procedure used to determine
