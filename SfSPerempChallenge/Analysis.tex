...
which were used to devise its form is presented in \ref{app:devmob}\footnote{Here it suffices to mention that much of its design
  ...
  perception.}. Instead, the most interesting patterns visible in the plot will be discussed here.
...
reproduced for the Sunshine capital cases, the Stubborn data, and the Philadelphia data. These plots can be see in
...
that commonly held opinions within racial groups are not the cause.

Of course, this result fails to account for a critical dependency between these juror rejections: the case being tried. Some
elements of the case have been accounted for here, for example the defendant race and gender, but there are obvious dependencies
which such broad categories do not control. Effects dependent on complex interactions between lawyers, the region of the trial,
and the charges laid against the defendant are sure to affect the observed rejection rates.

To account for these effects with a limited number of coefficients, and without overfitting, mixed models were used to fit a model
of rejection rates, using the \texttt{lme4} package in \Rp. This model was a very simple Poisson model with random 
