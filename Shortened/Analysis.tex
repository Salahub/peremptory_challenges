...
\begin{table}[h!]
  ...
  \caption[Strike Rate by Race]{\footnotesize The conditional probability of a venire member being struck peremptorily by the
    simplified venire member race across data sets. Note that the Philadelphia trial data only
...
\begin{figure}[!h]
  ...
  \caption[The ``Mobile Plot'' of Strikes by Racial Combination (Sunshine)]{\footnotesize The conditional probability of strike disposition given the
    venire member and defendant race. Expected values are represented by the horizontal black lines, and the observed values
...
to the conditional probability of a particular disposition given a race and defendant race combination. Racial combinations
...
expected probability of defence strike. The prosecution seems to mirror this tendency, striking the
...
sets immediately clear. Note that the restricted study sample universes of the Philadelphia and Stubborn data make these plots considerably different compared to Figure \ref{fig:racedefci}.

Despite the very different study populations of these three data sets, all display identical patterns, with only the
magnitudes of the strike rates differing. The mobile plot format illuminates several interesting aspects. The similar level of all black lines within each plot shows that in each data set, the aggregate probability of removal is
